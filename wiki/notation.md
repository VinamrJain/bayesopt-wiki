...
| Symbol | Meaning |
|---|---|
| $f$ | objective function, $f:A\to\R$, to be maximized; black-box, expensive, derivative-free |
| $A$ | feasible set, $A\subseteq\R^d$ (typically a hyper-rectangle or simplex) |
| $d$ | input dimension (BayesOpt targets $d\lesssim 20$) |
| $x,\,x'$ | points in $A$ |
| $x^*$ | a global maximizer, $x^*\in\argmax_{x\in A} f(x)$ |
...
| $\varepsilon_n$ | observation noise, i.i.d. $\Normal(0,\lambda)$ |
...
| Symbol | Meaning |
|---|---|
| $\alpha_n(x)$ | generic acquisition: value of sampling $x$ given $\mathcal{D}_n$; next point $x_{n+1}\in\argmax_x\alpha_n(x)$ |
...
| $\EI_n,\ \mathrm{PI}_n,\ \mathrm{UCB}_n,\ \KG_n,\ \ES_n,\ \PES_n,\ \mathrm{MES}_n$ | the specific acquisition functions (see their notes) |
...
| Symbol | Meaning |
|---|---|
| $Z$ | standard normal variable, $Z\sim\Normal(0,1)$ |
...
| $\argmax,\ \argmin$ | maximizing / minimizing argument |
| $\Normal(m,\Sigma)$ | (multivariate) normal with mean $m$, covariance $\Sigma$ |
